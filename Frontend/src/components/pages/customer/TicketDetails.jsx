...
const TicketDetails = () => {
  ...
  useEffect(() => {
    ...
    // Set up an auto-refresh interval for resolved tickets - optimized to reduce auth overhead
    let intervalId;
    if (ticket && ticket.status === 'resolved') {
      console.log('Setting up auto-refresh for resolved ticket (2 min interval)');
      ...
      }, 120000); // Changed from 10 seconds to 2 minutes to reduce auth overhead
    }
  ...
  const handleRatingSubmitted = async (ratingData) => {
    try {
      ...
      // Try to fetch the actual rating from the server to ensure we have complete data
      try {
      ...
      if (ratingResponse && ratingResponse.success && ratingResponse.rating) {
        setTicketRating(ratingResponse.rating);
          ...
          // If we can't fetch it, use what we submitted plus the response data
          setTicketRating({
            rating: ratingData.rating,
            feedback: ratingData.feedback,
            ...response.rating // Include any additional data from the submit response
          });
        }
      } catch (fetchError) {
        console.log('Could not fetch rating after submission, using submitted data:', fetchError);
        // If fetching fails, still use the submitted data to show success
        setTicketRating({
          rating: ratingData.rating,
          feedback: ratingData.feedback,
          ...response.rating // Include any additional data from the submit response
