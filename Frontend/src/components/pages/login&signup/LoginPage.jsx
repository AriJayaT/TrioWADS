// src/components/pages/login&signup/LoginPage.jsx
...
const Star = ({ 
  ...
  return (
    <svg 
      ...
      viewBox="0 0 24 24"
      className={`${position} ${size} ${className} pointer-events-none`}
      style={{
...
const LoginPage = () => {
  return (
    <div className="relative w-full min-h-screen bg-gradient-to-r from-white to-pink-200 overflow-hidden flex items-center justify-center p-4">      {/* Decorative Circles - Only visible on medium screens and up */}
      <div className="absolute w-7 h-7 bg-black rounded-full bottom-[23%] left-[17%] hidden md:block pointer-events-none" style={{zIndex: 5}} />
      <div className="absolute w-9 h-9 bg-pink-300 rounded-full top-[40%] left-[24%] hidden md:block pointer-events-none" style={{zIndex: 5}} />
      <div className="absolute w-20 h-20 bg-pink-300 rounded-full bottom-[25%] right-[13%] hidden md:block pointer-events-none" style={{zIndex: 5}} />
      <div className="absolute w-7 h-7 bg-pink-300 rounded-full bottom-[37%] right-[19%] hidden md:block pointer-events-none" style={{zIndex: 5}} />
      ...
      <div 
        className="absolute w-18 h-18 border-2 border-pink-300 bg-transparent top-[23%] right-[12.5%] hidden md:block pointer-events-none"
        style={{ zIndex: 10 }}
      /> 
      <div 
        className="absolute w-9 h-9 border-2 border-pink-300 bg-transparent bottom-[28%] right-[36.5%] hidden md:block pointer-events-none"
        style={{ zIndex: 5 }}
      /> 

      {/* Plus Sign - Only visible on medium screens and up */}
      <div className="absolute top-[12%] left-[1.7%] text-black text-7xl font-light hidden md:block pointer-events-none" style={{zIndex: 10}}>
        +
      </div>
      ...
      {/* Main login form card - Added z-index to ensure it's above decorative elements */}
      <div className="relative z-20">
        <LoginForm />
      </div>
      
      {/* Additional plush images that may overlap the form, place them below the form in z-index */}
      <ResponsivePlushImage 
        src={bunny} 
        alt="Bunny Plush" 
