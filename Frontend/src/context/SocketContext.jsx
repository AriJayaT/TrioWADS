...
export const SocketProvider = ({ children }) => {
  ...
  const subscribeToEvent = useCallback((eventName, handler, componentName = 'unknown') => {
    ...
    console.log(`[Socket] Handler stored for ${eventName} from ${componentName}, will be registered when socket connects`);
  }, []);
  ...
  useEffect(() => {
    ...
    const socketInstance = io(SOCKET_URL, {
      ...
      timeout: 60000,        // 1 minute (increased from 20 seconds)
      autoConnect: true,
      forceNew: false,       // Don't force new connection each time
    ...
    socketInstance.on('connect', () => {
      ...
      setTimeout(() => {
        console.log('[Socket] Authenticating with user ID:', user.id);
    ...
    socketInstance.on('authenticated', (data) => {
      ...
      setTimeout(() => {
        console.log('[Socket] Sending test ping after authentication');
        ...
        handlerMap.forEach((handlerSet, componentName) => {
          handlerSet.forEach((handler) => {
            const wrappedHandler = (...args) => {
              try {
                console.log(`[Socket] Processing ${eventName} event for ${componentName}:`, {
                  eventData: args[0],
    ...
    // Add ping/pong event logging for debugging
    socketInstance.on('ping', () => {
      console.log('[Socket] Ping received from server');
    });

    socketInstance.on('pong', (latency) => {
      console.log('[Socket] Pong sent to server, latency:', latency, 'ms');
    });
  ...
  useEffect(() => {
    ...
    eventHandlersRef.current.forEach((handlerMap, eventName) => {
      handlerMap.forEach((handlerSet, componentName) => {
        handlerSet.forEach((handler) => {
          const wrappedHandler = (...args) => {
            try {
              console.log(`[Socket] Received ${eventName} event for ${componentName}:`, args[0]?._id || 'no-id');
              handler(...args);
            } catch (error) {
              console.error(`[Socket] Error in ${componentName} handler for ${eventName}:`, error);
            }
    ...
    console.log(`[Socket] ${handlerCount} handlers re-registered successfully`);
  }, [socket, isConnected]);
