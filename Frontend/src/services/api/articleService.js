...
const articleService = {
   ...
   * @param {string} ticketSubject - The ticket subject to match against
   * @param {string} ticketCategory - The ticket category to match against
   * @param {number} [limit=3] - Number of related articles to return
   * @returns {Promise<Object>} - Related articles
   */
  getRelatedArticles: async (ticketSubject, ticketCategory, limit = 3) => {
    try {
      const response = await apiClient.get('/articles/related', {
        params: {
      ...
      throw error.response?.data?.error || 'Failed to get related articles';
    }
  }
};
