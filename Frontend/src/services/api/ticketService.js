...
const ticketService = {
  ...
  /**
   * Get ticket distribution for dashboard widget
   * @returns {Promise<Object>} - Ticket distribution by status
   */
  getTicketDistribution: async () => {
    try {
      const response = await apiClient.get('/tickets/distribution');
      if (response.data.success && response.data.stats) {
        const stats = response.data.stats;
        
        // Map the backend status counts to frontend display format
        const distribution = {
          open: stats.statusCounts?.open || 0,
          inProgress: stats.statusCounts?.inProgress || 0,
          resolved: stats.statusCounts?.resolved || 0,
          pending: stats.statusCounts?.waitingForCustomer || 0,
      ...
      return {
        success: false,
        distribution: { open: 0, inProgress: 0, resolved: 0, pending: 0, closed: 0 }
      };
    } catch (error) {
      throw error.response?.data?.error || 'Failed to fetch ticket distribution';
    }
  },
  
  /**
   * Get agent-specific statistics (for agent dashboard)
   * @returns {Promise<Object>} - Agent-specific ticket statistics
   */
  getAgentStats: async () => {
    try {
      const response = await apiClient.get('/tickets/agent/stats');
      return response.data;
    } catch (error) {
      throw error.response?.data?.error || 'Failed to fetch agent statistics';
    }
  },
   ...
   * @param {number} [limit=10] - Number of recent ratings to fetch
   * @returns {Promise<Object>} - Recent ratings data
   */
  getRecentRatings: async (limit = 10) => {
    try {
      const response = await apiClient.get('/tickets/ratings/recent', { 
        params: { limit } 
      });
      return response.data;
    } catch (error) {
      throw error.response?.data?.error || 'Failed to fetch recent ratings';
    }
  }
};
